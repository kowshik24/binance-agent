...
class SearchQueriesModel(BaseModel):
    queries: List[str] = Field(..., description="A list of 5 web search queries to gather intelligence.")
    ...
    try:
        ...
        for asset in balances:
            ...
            try:
                ...
                sma50 = df.ta.sma(50).iloc[-1]
                sma200 = df.ta.sma(200).iloc[-1]
                ...
                perf_24h = (df_7d['close'].iloc[-1] / df_7d['close'].iloc[-2] - 1) * 100
                perf_7d = (df_7d['close'].iloc[-1] / df_7d['close'].iloc[0] - 1) * 100
                ...
                portfolio_data.append({
                    "Asset": coin, "USD Value": usd_value, "24h Perf (%)": perf_24h, "7d Perf (%)": perf_7d,
                    "RSI": rsi, "Price/SMA50": price / sma50, "SMA50/SMA200": sma50 / sma200,
                    "Market Cap": market_cap, "Sparkline": sparkline
                })

            except Exception as e:
                # Handle coins without USDT pairs or other errors
                if coin.endswith('UP') or coin.endswith('DOWN') or coin in ['USDT', 'USDC']:
                    continue # Skip leveraged tokens and stablecoins from this complex analysis
        
        if usdt_balance > 1.0:
            total_portfolio_value += usdt_balance
            portfolio_data.append({"Asset": "USDT", "USD Value": usdt_balance, "24h Perf (%)": 0, "7d Perf (%)": 0, "RSI": None, "Price/SMA50": None, "SMA50/SMA200": None, "Market Cap": None, "Sparkline": ""})
...
if st.session_state.get('stage') == 'analysis_running':
    with st.spinner("Step 2/3: The AI committee is deliberating based on your risk profile..."):
        ...
        **--- YOUR INSTRUCTIONS ---**
        1.  **Synthesize All Data:** Analyze the provided portfolio data, which includes fundamental (Market Cap), technical (RSI, Moving Averages), and performance metrics.
        2.  **Adhere to Risk Profile:**
            - **Conservative:** Prioritize capital preservation. Focus on large-caps (BTC, ETH). Suggest selling high-risk/volatile assets. Be very cautious with new buys.
            - **Balanced:** Seek a mix of growth and stability. Can include some promising mid-caps alongside core large-cap holdings. Rebalancing is key.
            - **Aggressive:** Aim for maximum growth. Can suggest smaller, high-volatility altcoins with strong narratives. Can take on more risk for higher potential rewards.
        3.  **Generate a Coherent Strategy:** Briefly summarize your overarching plan.
        4.  **Create Actionable Trades:**
            - For **BUY** actions, use the available USDT funds. Crucially, you **MUST** suggest an `entry_price`, a `take_profit_price`, and a `stop_loss_price`.
            - For **SELL** actions, recommend selling a portion or all of a holding to de-risk, take profits, or reallocate.
            - For **HOLD** actions, justify why the asset's risk/reward is appropriate for the user's profile.
...
if st.session_state.get('stage') == 'complete':
    ...
    # --- Tabbed Output ---
    tab1, tab2, tab3 = st.tabs(["🎯 Action Plan", "📊 Portfolio Deep-Dive", "🤖 Agent Reports (JSON)"])
    ...
    with tab1:
        if plan:
            ...
            actions_config = {"BUY": "🟢", "SELL": "🔴", "HOLD": "🟡"}
            for trade in plan.trade_recommendations:
                with st.container(border=True):
                    st.markdown(f"<h5>{actions_config[trade.action]} {trade.action} {trade.coin}</h5>", unsafe_allow_html=True)
                    if trade.amount_usd:
                    ...
                    if trade.action == "SELL":
                        st.warning("⚠️ **Note:** Selling this asset may create a taxable event. Consult a tax professional.", icon="💰")
            
            # --- What-If Scenario Analysis ---
            st.header("🔬 What-If Scenario Analysis")
            with st.form("what_if_form"):
                what_if_query = st.text_area("Ask a follow-up question to the strategist:", placeholder="e.g., 'Generate an alternative plan that only invests in AI-related coins.' or 'What if I want to be more conservative and only hold Bitcoin?'")
                ...
                if submitted and what_if_query:
                    with st.spinner("The Chief Strategist is re-evaluating..."):
                        what_if_prompt = f"""
                        The user has received an initial investment plan. Now, they want to explore an alternative based on a new request.
                        
                        **Original Portfolio Data:**
                        {st.session_state.portfolio_summary['dataframe'].to_json(orient='records', indent=2)}
                        
                        **User's Risk Profile:** {st.session_state.risk_profile}
                        **Available Funds:** ${st.session_state.available_funds:,.2f}
                        
                        **USER'S NEW REQUEST:** "{what_if_query}"
                        
                        **Your Task:** Generate a *new* `FinalInvestmentPlan` that directly addresses the user's new request, while still using the original portfolio data and risk profile as context.
                        """
                        alternative_plan = get_llm_response(what_if_prompt, st.session_state.openai_api_key, FinalInvestmentPlan)
                        st.session_state.alternative_plan = alternative_plan
            
            if 'alternative_plan' in st.session_state:
                st.subheader("Alternative Plan")
                alt_plan = st.session_state.alternative_plan
                st.write(alt_plan.strategy_summary)
                for trade in alt_plan.trade_recommendations:
                     st.markdown(f"- **{trade.action} {trade.coin}**: {trade.reasoning}")
    ...
    with tab2:
        ...
        st.markdown(
            df_display[['Asset', 'USD Value', '24h Perf (%)', '7d Perf (%)', 'Sparkline HTML', 'RSI', 'Price/SMA50', 'SMA50/SMA200', 'Market Cap']].to_html(escape=False, formatters={
                '24h Perf (%)': '{:,.2f}%'.format,
                '7d Perf (%)': '{:,.2f}%'.format,
                'RSI': '{:,.1f}'.format,
    ...
    with tab3:
        st.header("Raw Agent Output (JSON)")
        ...
        st.download_button(
            ...
            data=pdf_data,
            file_name=f"AI_Investment_Report_{datetime.now().strftime('%Y%m%d')}.pdf",
